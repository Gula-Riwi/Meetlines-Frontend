<template>
    <aside class="w-64 h-screen bg-gray-900 border-r border-white/10 flex flex-col text-white transition-all duration-300 hidden md:flex z-50 relative">
        ...
        <div class="h-20 flex items-center justify-center border-b border-white/10 px-4">
            <div class="text-center w-full">
                ...
                <h3 class="text-xl font-bold font-league tracking-wide truncate text-white">
                    {{ currentProject.name || 'Mi Proyecto' }}
                </h3>
                <!-- Subtítulo o link -->
                <p v-if="currentProject.subdomain" class="text-xs text-indigo-400 truncate">
                    {{ currentProject.subdomain }}.meetlines.com
                </p>
            </div>
        </div>
        ...
        <div class="px-4 mb-2">
            <button @click="changeProject" class="w-full py-2 px-3 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white rounded-lg text-xs font-bold border border-white/5 transition-colors flex items-center justify-center gap-2">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3">
            <ul class="space-y-2">
                <li>
                    <router-link to="/dashboard" active-class="bg-indigo-600 shadow-lg shadow-indigo-500/20" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 transition-all">
                        <font-awesome-icon :icon="['fas', 'check']" class="text-lg" />
                        <span class="font-medium">Resumen</span>
                    </router-link>
                </li>
                <li>
                    <router-link to="/profile" active-class="bg-indigo-600 shadow-lg shadow-indigo-500/20" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 transition-all">
        ...
        <div class="p-4 border-t border-white/10">
            <button @click="Logout" class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all duration-200">
